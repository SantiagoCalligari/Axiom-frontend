...
import { Check, Download, MessageCircle, Send, User, X } from 'lucide-react';
...
export default async function ExamPage({ params }: { params: Promise<{ universitySlug: string, careerSlug: string, subjectSlug: string, examId: string }> }) {
  ...
  return (
    <div className="container mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Card className="overflow-hidden">
            ...
            <CardContent>
              {pdfUrl ? (
                ...
                >
                  Tu navegador no soporta iframes. Puedes <a href={pdfUrl} target="_blank" rel="noopener noreferrer" className="underline">descargar el archivo aquí</a>.
                </iframe>
              ) : (
        ...
        {/* Columna Derecha: Foro/Comentarios (Placeholder) */}
        <div className="lg:col-span-1">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-x-2">
                <MessageCircle className="h-5 w-5" /> Discusión / Comentarios
              </CardTitle>
              <CardDescription>Comparte tus dudas o aportes sobre este examen.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {/* Área para mostrar comentarios (Placeholder) */}
              <div className="h-64 overflow-y-auto border rounded-md p-3 space-y-3 bg-muted/30">
                <p className="text-sm text-muted-foreground text-center py-10">
                  (Aquí se mostrarán los comentarios del foro)
                  <br />
                  Funcionalidad pendiente.
                </p>
              </div>
              {/* Área para añadir comentario (Placeholder) */}
              <div className="space-y-2">
                <Label htmlFor="comment">Añadir comentario</Label>
                <Textarea id="comment" placeholder="Escribe tu comentario aquí..." rows={3} />
...
export async function generateMetadata({ params }: { params: Promise<{ universitySlug: string, careerSlug: string, subjectSlug: string, examId: string }> }) {
  const { universitySlug, careerSlug, subjectSlug, examId } = await params;
